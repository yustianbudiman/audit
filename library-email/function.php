...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'mail.mj-infomatika.my.id';  // Masukkan Server SMTP
  $mail->Port 		  = 465;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'admin@mj-infomatika.my.id';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= '123456P@ssw0rd';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'admin@mj-infomatika.my.id';        // Masukkan default from pada email
