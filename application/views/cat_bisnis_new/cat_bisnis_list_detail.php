                ...
                <div class="box box-info">
                    <div class="box-header">
                        <h3 class="box-title">MANAGE
                            <small>Data CAT Bisnis Detail</small>
                        </h3>
                        <!-- tools box -->
                    ...
                    <div class="box-body pad">
                        ...
                        <?php echo anchor(site_url('cat_bisnis/create/'.$one_header_detail['id_cat_bisnis_header']), '<i class="fa fa-wpforms" aria-hidden="true"></i> Tambah Data', 'class="btn btn-danger btn-sm"'); ?>
                        <?php echo anchor(site_url('cat_bisnis/excel/'.$one_header_detail['id_cat_bisnis_header']), '<i class="fa fa-file-excel-o" aria-hidden="true"></i> Export Ms Excel', 'class="btn btn-success btn-sm"'); ?>
                        <?php echo anchor(site_url('cat_bisnis/word/'.$one_header_detail['id_cat_bisnis_header'].'/'.$one_header_detail['periode'].'/'.$one_header_detail['periode'].'/'.$one_header_detail['periode']), '<i class="fa fa-file-word-o" aria-hidden="true"></i> Export Ms Word', 'class="btn btn-warning btn-sm"'); ?>
                    </div>
                    <table class="table table-bordered table-hover" id="dteail_cat_bisnis" style="font-size: 12px;">
                        ...
                        <tbody>
                            <?php $no=1; foreach ($list_all_cat_bisnis as $key) { ?>
                            <tr>
                                <td><?php echo $no;?></td>
                                ...
                                <td style="text-align: center;">
                                    ...
                                    <a href="<?php echo base_url('cat_bisnis/update/'.$key['id_cat_bisnis']);?>" class="btn btn-primary btn-xs btn-flat <?php echo ($key['status']==2?'disabled':'');?>"><i class="fa fa-edit"></i> Edit</a>
                                    <button type="button" class="btn btn-danger btn-xs btn-flat hapus_cat_binis" <?php echo ($key['status']==2?'disabled':'');?> data-id_cat_binis="<?php echo $key['id_cat_bisnis']?>" data-id_cat_binis_header="<?php echo $key['id_cat_bisnis_header']?>" data-toggle="modal" data-target="#ModalDelete"><i class="fa fa-trash"></i> Hapus</button>
                                </td>
                            </tr>
                            <?php $no++; } ?>
                        </tbody>
...
<div id="ModalDelete" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <form class="form-horizontal" action="<?php echo base_url('cat_bisnis/delete');?>" method="POST">
      <div class="modal-body">
        <p>Apkah anda yakin akan menghapus data ini</p>
        <input type="hidden" name="MD_id_cat_bisnis_header" id="MD_id_cat_bisnis_header">
        <input type="hidden" name="MD_id_cat_bisnis" id="MD_id_cat_bisnis">
      </div>
...
<script type="text/javascript">
    $(document).on('click','.hapus_cat_binis',function(){
        var id_cat_bisnis_header=$(this).attr('data-id_cat_binis_header');
        var id_cat_bisnis=$(this).attr('data-id_cat_binis');
        $('#MD_id_cat_bisnis_header').val(id_cat_bisnis_header);
        $('#MD_id_cat_bisnis').val(id_cat_bisnis);
    });
</script>