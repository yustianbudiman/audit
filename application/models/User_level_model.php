...
class User_level_model extends CI_Model
{
    ...
    function json() {
        ...
        if($this->session->userdata('id_user_level') == 1){ // super admin
            ...
            $this->datatables->add_column('action', anchor(site_url('userlevel/akses/$1'),'<i class="fa fa-eye" aria-hidden="true"></i>', array('class' => 'btn btn-danger btn-sm'))." 
                ".anchor(site_url('userlevel/update/$1'),'<i class="fa fa-pencil-square-o" aria-hidden="true"></i>', array('class' => 'btn btn-danger btn-sm')), 'id_user_level');
        }else{
            $this->datatables->where('id_user_level !=','1');
            $this->datatables->where('id_user_level !=','2');
            $this->datatables->add_column('action', anchor(site_url('userlevel/akses/$1'),'<i class="fa fa-eye" aria-hidden="true"></i>', array('class' => 'btn btn-danger btn-sm')), 'id_user_level');
        }
