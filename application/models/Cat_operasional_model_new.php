...
class Cat_operasional_model_new extends CI_Model
{

    public $table = 'cat_operasional_new';
    ...
    // datatables
    function json() {
        $this->datatables->select('cat_operasional_new.id_cat_operasional,cat_operasional_new.id_cabang,cat_operasional_new.temuan,cat_operasional_new.klasifikasi_temuan,cat_operasional_new.kriteria,cat_operasional_new.dampak,cat_operasional_new.tev,cat_operasional_new.bobot_resiko,cat_operasional_new.rekomendasi,cat_operasional_new.tanggapan_audit,cat_operasional_new.target_date,cat_operasional_new.status,status_trx.status_trx,cat_operasional_new.created_date,cat_operasional_new.created_by,klasifikasi_temuan.nama_klasifikasi_temuan,cabang.kode_cabang,cabang.nama_cabang,penyimpangan.nama_penyimpangan,tbl_user.full_name');
        $this->datatables->from('cat_operasional_new');
        //add this line for join
        $this->datatables->join('status_trx', 'cat_operasional_new.status = status_trx.id_status');
        $this->datatables->join('penyimpangan', 'cat_operasional_new.id_penyimpangan = penyimpangan.id_penyimpangan');
        $this->datatables->join('klasifikasi_temuan', 'cat_operasional_new.klasifikasi_temuan = klasifikasi_temuan.id_klasifikasi_temuan');
        $this->datatables->join('cabang', 'cat_operasional_new.id_cabang = cabang.id_cabang');
        $this->datatables->join('tbl_user', 'cat_operasional_new.created_by= tbl_user.id_users');
        if($this->session->userdata('id_user_level')=='6'){
            $this->datatables->where('cat_operasional_new.created_by', $this->session->userdata('id_users'));
        }
        $this->datatables->where('cat_operasional_new.aktif', 'Aktif');
        $this->datatables->add_column('action', anchor(site_url('cat_operasional_new/update/$1'),'<i class="fa fa-pencil-square-o" aria-hidden="true"></i>', array('class' => 'btn btn-danger btn-sm'))." 
                ".anchor(site_url('cat_operasional_new/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id_cat_operasional,cat_operasional_new.id_cabang,nama_cabang');
    ...
    //     $this->datatables->add_column('action', anchor(site_url('cat_operasional/tambah_data/$1'),'<i class="fa fa-plus" aria-hidden="true"></i> Tambah', array('class' => 'btn btn-primary btn-xs btn-flat')), 'id_cabang');
    //     return $this->datatables->generate();
    //}
    ...
    function get_all()
    {
        $sql="SELECT  * from cat_operasional_new a
 ...
 //        $sql="SELECT
 //                a.id_cat_operasional,a.id_cat_operasional_header, a.temuan,a.kriteria, b.nama_klasifikasi_temuan,a.dampak, c.nama_penyimpangan,a.total_impact,a.repeated,a.tev,a.bobot_resiko,a.rekomendasi,a.tanggapan_audit,a.target_date,d.status_trx,a.member,a.status
 //            FROM
 //                cat_operasional a
 ...
 //        $sql="SELECT  a.*,b.full_name as created_name,c.full_name as updated_name from v_cat_operasional a
 ...
 //    }
    ...
    function delete_by_id($id){
        $this->db->where('id_cat_operasional', $id);
        $this->db->update('cat_operasional_new', array('status'=>5));
    }
...
/* End of file Catoperasional_model.php */
/* Location: ./application/models/Catoperasional_model.php */
