...
class Cat_operasional_new extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Cat_operasional_model_new');
    ...
    {   
        ...
        $this->template->load('template','cat_operasional_new/cat_operasional_list');
    } 

    public function json_cabang() {
        header('Content-Type: application/json');
        echo $this->Cat_operasional_model_new->json_cabang();
    }
  ...
  //               'list_all_cat_operasional'=>$this->Cat_operasional_model->get_all_Cat_Bisnis($header['id_cat_operasional_header']),
  //           ];
  ...
  //   }
    ...
    public function json() {
        header('Content-Type: application/json');
        echo $this->Cat_operasional_model_new->json();
    }

  //   public function read($id) 
  //   {
  //       $row = $this->Cat_operasional_model->get_One_Cat_Bisnis($id);
  //       if ($row) {
  //           $data = array(
  ...
  //   }
    ...
    public function create() 
    {
        ...
        // $header=$this->Cat_operasional_model->get_One_Header_detail($id);
 		...
 		$data=[
            'button' => 'Create',
            'action' => base_url('cat_operasional_new/create_action'),
        ...
        $this->template->load('template','cat_operasional_new/cat_operasional_form', $data);
    }
    ...
    {
        $row = $this->Cat_operasional_model_new->get_by_id($id);
        // $header=$this->Cat_operasional_model_new->list_cat_operasional_header($row->id_cat_operasional_header);
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => base_url('cat_operasional_new/update_action'),
                'list_cabang'=>$this->Cabang_model->get_all(),
                'list_tl'=>$this->User_model->get_All_userBy_level(),
                'list_supervisor'=>$this->User_model->get_All_userBy_level(array('5','4')),
                'list_audit'=>$this->User_model->get_All_userBy_level(array('6')),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Cat_operasional_model_new->update($this->input->post('id_cat_operasional',TRUE), $data);
    ...
    //     $MD_id_cat_operasional_header =$this->input->post('MD_id_cat_operasional_header',TRUE);
    //     $MD_id_cat_operasional =$this->input->post('MD_id_cat_operasional',TRUE);

    //     $row = $this->Cat_operasional_model->get_by_id($MD_id_cat_operasional);
    //     if ($row) {
    //         // $this->Cat_operasional_model->delete($id);
    //         $data=[
    //                 'aktif'=>'Nonaktif'
    //             ];
    //         $this->Cat_operasional_model->update($MD_id_cat_operasional, $data);
        ...
        public function delete($id) 
    {
        ...
        if ($row) {
            $this->Cat_operasional_model_new->delete_by_id($id);
            $this->session->set_flashdata('message',array('type'=>'alert-success','pesan'=>'Delete Record Success'));
            redirect(site_url('cat_operasional_new'));
        } else {
            $this->session->set_flashdata('message',array('type'=>'alert-warning','pesan'=>'Record Not Found'));
  ...
  //   }
    ...
    public function _rules() 
    {
		...
		$this->form_validation->set_rules('cat_operasional', 'id_cat_operasional', 'trim');
    ...
    public function excel()
    {
		...
		xlsWriteLabel($tablehead, $kolomhead++, "Goal Stategic");
		...
		foreach ($this->Cat_operasional_model_new->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word(){
        $data=[
                'list_all_cat_operasional'=>$this->Cat_operasional_model_new->get_all(),
            ];
        $this->load->view('cat_operasional_new/generate_docx',$data);
    }
