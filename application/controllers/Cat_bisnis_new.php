...
class Cat_bisnis_new extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Cat_bisnis_model_new');
    ...
    {   
        ...
        $this->template->load('template','cat_bisnis_new/cat_bisnis_list');
    } 

    public function json_cabang() {
        header('Content-Type: application/json');
        echo $this->Cat_bisnis_model_new->json_cabang();
    }
  ...
  //   }
    ...
    public function json() {
        header('Content-Type: application/json');
        echo $this->Cat_bisnis_model_new->json();
    }
  ...
  //   }
    ...
    public function create() 
    {
        ...
        // $header=$this->Cat_bisnis_model->get_One_Header_detail($id);
 		...
 		$data=[
            'button' => 'Create',
            'action' => base_url('cat_bisnis_new/create_action'),
        ...
        $this->template->load('template','cat_bisnis_new/cat_bisnis_form', $data);
    }
    ...
    public function create_action() 
    {
       ...
       //     $this->create();
        //} else {
            ...
            $this->Cat_bisnis_model_new->insert($data);
    ...
    {
        $row = $this->Cat_bisnis_model_new->get_by_id($id);
        // $header=$this->Cat_bisnis_model_new->list_cat_bisnis_header($row->id_cat_bisnis_header);
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => base_url('cat_bisnis_new/update_action'),
                'list_cabang'=>$this->Cabang_model->get_all(),
                'list_tl'=>$this->User_model->get_All_userBy_level(),
                'list_supervisor'=>$this->User_model->get_All_userBy_level(array('5','4')),
                'list_audit'=>$this->User_model->get_All_userBy_level(array('6')),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Cat_bisnis_model_new->update($this->input->post('id_cat_bisnis',TRUE), $data);
        ...
        public function delete($id) 
    {
        ...
        if ($row) {
            $this->Cat_bisnis_model_new->delete_by_id($id);
            $this->session->set_flashdata('message',array('type'=>'alert-success','pesan'=>'Delete Record Success'));
            redirect(site_url('cat_bisnis_new'));
        } else {
            $this->session->set_flashdata('message',array('type'=>'alert-warning','pesan'=>'Record Not Found'));
  ...
  //               ];
  //           $this->Cat_bisnis_model->save_log($data_log);
  //           }
  ...
  //   }
    ...
    public function excel()
    {
		...
		foreach ($this->Cat_bisnis_model_new->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word(){
        $data=[
                'list_all_cat_bisnis'=>$this->Cat_bisnis_model_new->get_all(),
            ];
        $this->load->view('cat_bisnis_new/generate_docx',$data);
    }
